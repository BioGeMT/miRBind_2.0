...
def train_model(data, labels, dataset_size, ratio, model_file):
...
def main():
    ...
    parser.add_argument('--dataset_size', type=int, required=True, help="Number of samples in the dataset. Needed to properly load the nump files.")
    parser.add_argument('--model', type=str, required=False, help="Filename to save the trained model")
    args = parser.parse_args()

    if args.model is None:
        args.model = f"model_Manakov22_{args.ratio}.keras"

    start = time.time()
    train_model(args.data, args.labels, args.dataset_size, args.ratio, args.model)
    end = time.time()
