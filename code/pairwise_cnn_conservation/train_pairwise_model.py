...
def train_epoch(model, train_loader, optimizer, criterion, device):
    ...
    for batch_X, batch_phylop, batch_phastcons, batch_y in train_loader:
        ...
        optimizer.zero_grad()
        outputs = model(batch_X, batch_phylop, batch_phastcons).squeeze()
...
def main():
    parser = argparse.ArgumentParser(description='Train miRNA pairwise binding site prediction model with conservation scores')
    ...
    # Validate that the number of filters and kernels matches
    if len(filter_sizes) != 3 or len(kernel_sizes) != 3:
        print(f"Warning: Number of filter sizes ({len(filter_sizes)}) or kernel sizes ({len(kernel_sizes)}) "
              f"doesn't match the expected 3 convolutional layers")
        print("Adjusting lists to match...")
        
        # Adjust filter_sizes if needed
        if len(filter_sizes) < 3:
            filter_sizes.extend([filter_sizes[-1]] * (3 - len(filter_sizes)))
        elif len(filter_sizes) > 3:
            filter_sizes = filter_sizes[:3]
            
        # Adjust kernel_sizes if needed
        if len(kernel_sizes) < 3:
            kernel_sizes.extend([kernel_sizes[-1]] * (3 - len(kernel_sizes)))
        elif len(kernel_sizes) > 3:
    ...
    # Load test datasets with conservation
    test_dataset1 = MiRNAConservationDataset(
        args.test_file1,
        args.target_length,
        args.mirna_length,
        pair_to_index,
    ...
    test_dataset2 = MiRNAConservationDataset(
        args.test_file2,
        args.target_length,
        args.mirna_length,
        pair_to_index,
    ...
    model_save_path = os.path.join(args.output_dir, f"pairwise_conservation_model_{timestamp}.pt")
